...
// 보장: 닫으면 열기 전에 포커스를 갖고 있던 요소로 돌아온다. 그 요소가 사라졌으면 아무 데도
//       옮기지 않는다.
// 경계: 열려 있는 동안의 포커스 이동은 보지 않는다 — 표준 <dialog>의 몫이다.

function Harness({ open, opener = true }: { open: boolean; opener?: boolean }) {
  return (
    <>
      {opener && (
...
it("열기 전 요소가 사라졌으면 포커스를 옮기지 않는다", async () => {
  const { getByTestId, rerender } = render(<Harness open={false} />);
  ...
  rerender(<Harness open={false} opener={false} />);
