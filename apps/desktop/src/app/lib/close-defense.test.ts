...
// 집행: file-lifecycle.md#종료-방어 — "경로가 있고 정규화 승인이 필요 없는 dirty 탭은 플러시,
//       Untitled dirty 탭이 있으면 다이얼로그".
// 왜: 이 분류가 틀리면 종료가 데이터를 버리거나(유실), 매번 다이얼로그로 사용자를 막는다.
// 보장: 탭 상태 → {플러시 대상, 종료를 막는 탭}의 결정론적 분류.
// 경계: 실제 저장·창 닫기(비동기 흐름·플러시 실패 처리)는 use-close-guard·E2E 소관.
//       M1은 정규화 미승인 탭이 존재하지 않는다(비UTF-8·혼합 EOL은 열기 자체를 거부).
