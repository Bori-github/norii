...
describe("handleFileRemoved", () => {
  ...
  //     성공 뒤에 도착하면, 멀쩡한 파일에 삭제 표시 + 자동 저장 정지가 남는다.
  ...
  // 왜: 지연이 빠지면 재확인이 재생성 저장보다 먼저 실행되어 고친 버그가 소리 없이
  //     회귀한다 — 이 테스트가 그 회귀를 고정한다.
...
describe("syncWatchedPaths", () => {
  ...
  //     고착된다(최악은 전부 건너뛰어 감시가 통째로 꺼진 채 성공으로 기억).
