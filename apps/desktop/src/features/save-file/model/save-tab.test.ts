...
// 경계: Rust가 실제로 비UTF-8을 열어 주는 것은 Rust 인코딩 변환 소관 — 여기는 메타가
...
describe("정규화 승인 게이팅", () => {
  ...
  //       한다 — 아니면 디스크는 UTF-8인데 배너가 유령처럼 남는다.
...
//       + "saved" 후 dirty 재확인 없이 닫으면 저장 중 편집이 유실된다.
...
//       이 불변식을 지켜야 한다(같은 파일 두 탭은 서로를 파괴한다).
...
describe("저장 경로 확정과 중복 탭 금지", () => {
  ...
  //     같은 파일이 두 신원을 갖지 않는다(파일 트리라는 두 번째 열기 입구에서도 같다).
