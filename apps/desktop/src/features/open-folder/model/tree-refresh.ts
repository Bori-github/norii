...
// 트리 외부 변경 반영 — 이벤트 계약: rust-commands.md watch_tree,
// 반영 정책: document-model.md#파일-트리-사이드바.
 ...
 * 선언은 직렬화하고 대기분은 latest-wins, 실패는 캐시 무효화로 재시도한다.
 */
...
/**
 * dir-changed 처리 — 읽어 둔 폴더만 한 단계를 다시 읽어 병합한다
 * (→ document-model.md#파일-트리-사이드바). 재읽기 실패는 조용히 넘어간다 —
 * 폴더 자체가 사라진 경우라면 부모의 dir-changed가 목록에서 지워 준다.
 */
...
/**
 * tree-desynced 처리 — 읽어 둔 모든 레벨을 다시 읽어 병합한다(→ rust-commands.md watch_tree).
