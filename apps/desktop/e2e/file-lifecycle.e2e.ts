...
// 집행: file-lifecycle.md#외부-변경-처리 — "비활성 탭의 충돌 표시(M2 확정)": 비활성 탭
//       충돌은 ⚠ 배지가 유일한 신호이고, 그 탭으로 전환하면 충돌 배너가 뜬다.
// 왜: 배너는 활성 탭 전용이라 이 배지가 없으면 사용자가 충돌을 알아차릴 방법이 없다 —
//     계약으로 확정된 동작인데 어떤 테스트도 커버하지 않았다(리뷰 지적).
...
it("사이드바 — 폴더를 열면 트리가 뜨고, 펼침과 파일 클릭이 실제로 동작한다", async () => {
  ...
  await (await browser.$("button=readme.md")).waitForExist({ timeout: 5_000 });

  // 폴더 펼침(lazy 읽기) → 하위 파일 클릭 → 탭이 열린다.
  await (await browser.$("button=notes")).click();
  const nested = await browser.$("button=folding.md");
...
it("사이드바 — 외부 생성이 트리에 나타나고 외부 삭제가 사라진다", async () => {
  ...
  const created = await browser.$("button=새파일.md");
  ...
  const foldingStillVisible = await (await browser.$("button=folding.md")).isExisting();
