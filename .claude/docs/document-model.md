...
- 항목 조작(새 파일·새 폴더·이름 변경·삭제)의 커맨드 계약은 [Rust 커맨드 계약](rust-commands.md#항목-조작)이 소유한다. 사이드바 쪽 규칙은 아래와 같다.
  - **헤더의 `+파일`·`+폴더` 버튼은 루트에 만든다.** 하위 폴더에 만들 때는 그 항목의 컨텍스트 메뉴를 쓴다.
  - **항목을 우클릭하면 메뉴가 열린다** — 새 파일·새 폴더·이름 변경·휴지통으로 이동. 새로 만드는 자리는 폴더면 그 안, 파일이면 그 부모다. **빈 영역을 우클릭하면 새 파일·새 폴더만** 열리고 루트에 만든다. 메뉴는 `role=menu`이고 방향키로 옮기며 `Esc`·바깥 누르기로 닫는다.
  - 이름은 **트리 안 입력칸**에서 받는다. 한 번에 하나만 열린다. 만들 자리에 입력칸이 나타나고, 접힌 폴더는 펼쳐진다.
  - 입력칸은 **기본 이름이 채워진 채**로 열린다(`새 파일.md`·`새 폴더`, 겹치면 빈 번호를 앞에서부터). 확장자를 뺀 앞부분만 선택돼 있어 바로 타이핑하면 이름만 바뀐다.
  - 입력 중 이름을 판정해 확정을 막고(빈 이름·`.` 시작·`/`·중복) 입력칸 아래에 알린다. 최종 판정은 Rust다.
  - `Enter` 확정, `Esc` 취소. 입력칸 밖을 누르면 확정하되, 확정할 수 없는 이름이면 취소한다.
  - 파일을 만들면 그 파일을 탭으로 연다. 폴더는 열지 않는다.
...
- 펼친 노드는 전부 렌더한다. 한 번 읽은 children은 접혀도 DOM에 남는다 — 숨김이라 포커스·키보드 탐색에서 빠진다(이유 → [모션](design/decisions/motion.md)). 가상화는 현재 두지 않는다.
...
열린 문서가 하나도 없어도 **탭바는 "새 탭" 하나를 표시한다.** 그 탭의 내용 영역이 곧 빈 상태 안내이고, 문서를 열면 **그 자리를 새 문서가 차지한다.**

빈 상태 안내는 버튼이다 — 폴더 열기 · 파일 열기 · 새 문서, 이 순서다. 파일 열기·새 문서에는 단축키를 병기한다.

- **새 문서 버튼은 `Cmd+N`과 같은 길이다** — 경로 없는 Untitled 탭을 만들고, 경로는 첫 저장 다이얼로그가 정한다(→ [다중 탭 규칙](#다중-탭-규칙)).
- 사이드바의 새 파일(실파일 즉시 생성)은 이 화면에 두지 않는다 — 폴더가 없으면 만들 대상 폴더가 없다.
...
- **앱에서 지우거나 이름을 바꾼 파일도 목록에서 변경되지 않는다** — 지운 파일의 항목이 남고, 이름을 바꾼 파일은 옛 경로로 남는다. 누르면 열지 못했다고 알린다. 없는 경로가 걸러지는 것은 다음 실행이다.
...
- 파일명으로 표시하고, 누르면 그 파일을 연다. 목록이 비면 영역을 표시하지 않는다. 폴더를 열지 않아도 표시된다.
- 헤더로 접고 펼친다. 접으면 헤더만 하단(설정 영역 위)에 남는다. 접힘은 영속화하지 않는다(트리 펼침과 동일 원칙).
- 목록은 **하나의 Tab 정지점**이다(roving tabindex) — `↑↓`·`Home`/`End`로 항목을 옮기고 `Enter`로 연다.
