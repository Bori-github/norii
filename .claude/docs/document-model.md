...
- 트리는 ARIA 트리(`role=tree`/`treeitem`/`group`)다 — 보이는 노드 중 하나만 Tab 정지점(roving tabindex)이고, `↑↓`·`←→`(펼침/접힘·부모/자식)·`Home`/`End`·`Enter`로 포인터 없이 탐색한다. ARIA 시맨틱과 키보드 탐색은 한 세트다(→ [작업 규칙 — 품질 게이트](../rules/project-rules.md)).
- 펼친 폴더가 비어 있으면 "비어 있음"을 표시한다.
- 사이드바를 접으면 화면에서 사라진다 — 아이콘 레일을 남기지 않는다. 타이틀 스트립의 토글과 단축키로 여닫는다(→ [에디터 전략 — 단축키 계약](editor-strategy.md#단축키-계약)). 접기는 트리의 펼침 상태(`expandedDirs`)를 바꾸지 않는다.
