...
빈 상태 안내는 버튼과 최근 파일 목록으로 구성된다.

- **버튼** — 폴더 열기 · 파일 열기 · 새 문서, 이 순서다. 파일 열기·새 문서에는 단축키를 병기한다.
...
- **최근 파일 목록** — `recentFiles`(→ [최근 파일](#최근-파일))를 파일명으로 표시하고, 누르면 그 파일을 연다. 목록이 비면 표시하지 않는다.
