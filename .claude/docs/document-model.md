...
- **외부 생성/삭제는 폴더 감시가 반영한다.** 루트를 재귀 감시(`watch_tree`)하고, `dir-changed`를 받은 프론트는 **그 폴더를 이미 읽어 둔 경우에만**(children 보유) 그 한 단계를 `read_dir`로 다시 읽어 병합한다 — 안 읽은 폴더는 다음 펼침이 어차피 최신을 읽는다. 병합은 살아남은 폴더의 기존 children·펼침 상태를 경로 기준으로 보존한다(재읽기가 하위 트리를 접어버리지 않게). 열린 파일의 watch(파일 내용)와 별개 문제다(→ [Rust 커맨드 계약](rust-commands.md) `watch_tree`).
