...
스타일은 **Panda CSS**로 작성하고, 디자인 시스템을 프로젝트 내부에 구축한다. 토큰과 디자인 시스템 컴포넌트(recipe 포함)는 `packages/ui`가 갖고, 앱의 `panda.config.ts`가 그 preset을 확장하며 `shared/ui`가 컴포넌트를 재노출한다. 상위 레이어는 `shared/ui`만 소비하고 직접 스타일을 최소화한다. 상세는 [디자인 시스템](design/design-system.md)을 단일 출처로 둔다.
