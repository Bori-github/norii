...
**이미지 권한(asset 프로토콜)** — 프리뷰의 로컬 이미지는 파일 커맨드가 아니라 Tauri asset 프로토콜로 읽고, 이 경로는 Tauri가 스코프 객체로 검사한다. 설정(`tauri.conf.json`의 `app.security.assetProtocol`)에는 `enable`만 켜고 `scope`는 비워 둔다 — 허용 경로는 위 세 입구에서 런타임에 정해지기 때문이다. **두 스코프의 단일 출처는 허용 루트 목록이다** — `FileScope`의 허용·거부가 asset 스코프에도 같은 경로를 등록한다(폴더는 하위 트리 전체, 파일은 그 파일만, 거부 디렉터리는 거부로). 갈라지면 파일 커맨드가 막는 경로를 이미지로 읽을 수 있으므로, 일치는 테스트가 지킨다.
