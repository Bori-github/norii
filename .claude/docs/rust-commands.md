...
// 지금 없는 경로(밖에서 지웠거나 옮긴 파일)는 결과에서 빠지고 허용되지도 않는다.
// 종류가 어긋난 경로도 빠진다 — 탭 경로는 파일이어야 하고 root_dir는 디렉터리여야 한다.
//   허용은 하위 트리 전체를 여므로, 탭 자리의 디렉터리 하나가 전역 접근으로 번진다.
...
**그래서 앱 자신의 config 디렉터리는 파일 커맨드가 만질 수 없다.** 사용자가 홈 폴더를 열면 그 디렉터리도 허용 루트 하위가 되고, 세션 파일을 `save_file`로 고칠 수 있으면 위 문장이 무너진다. 그 디렉터리는 허용보다 강한 거부 목록에 들어간다(`FileScope::deny`, setup에서 등록). 세션 파일을 신뢰하는 범위는 [보안 — 세션 파일](security.md#세션-파일)이 소유한다.
