...
| 프리뷰 파서 타입       | `@types/markdown-it`                                 | 14.1.2                                               | markdown-it 타입 정의 (본체는 타입 미포함)                                                                            |
| Sanitizer              | DOMPurify (`dompurify`)                              | 3.4.11                                               | XSS 방어 (필수)                                                                                                       |
| 프리뷰 수식            | KaTeX (`katex`)                                      | 0.16.47                                              | 수식 조판. CSS·폰트 로컬 번들, ESM 빌드 고정 — 앱과 패키지가 **같은 버전**이어야 한다(CSS는 앱이, 렌더러는 패키지가 소유) (→ [프리뷰 전략](preview-strategy.md#수식-katex)) |
| 수식 플러그인          | `@vscode/markdown-it-katex`                          | 1.1.2                                                | markdown-it ↔ KaTeX 연결 — Microsoft 유지보수 포크(원본은 사실상 대체됨)                                              |
| 프리뷰 각주            | `markdown-it-footnote`                               | 4.0.0                                                | 각주 — 참조·목록이 문서 내 앵커로 왕복 (→ [프리뷰 전략](preview-strategy.md#각주))                                    |
| 각주 플러그인 타입     | `@types/markdown-it-footnote`                        | 3.0.4                                                | markdown-it-footnote 타입 정의 (본체는 타입 미포함)                                                                   |
| 프리뷰 다이어그램      | Mermaid (`mermaid`)                                  | 11.16.0                                              | 클라이언트 SVG 렌더 · 다이어그램 있을 때만 lazy-load. sanitize 경계가 엔진 내부 sanitizer에 얹혀 있어 **정확 핀 필수** (→ [프리뷰 전략](preview-strategy.md#다이어그램-mermaid)) |
...
> 이 표는 **실제 설치된 핀**의 단일 출처다 — 설계 문서가 채택을 예고한 의존성(예: [에디터 전략](editor-strategy.md#확장-구성-초기)의 자동완성)이라도 설치 전까지 버전 열에 넣지 않는다(→ `docs-drift` 검사가 표↔핀을 단방향 대조).
...
이 표는 **실제 설치된 핀만** 등재한다 — 설계 문서가 예고한 플러그인(세션 저장 · 창 상태 복원 · 자동 업데이트 → [문서 모델 — 세션 복원](document-model.md#세션-복원) · [플랫폼 전략](platform-strategy.md))은 도입하는 커밋에서 여기에 추가한다.
...
| `plugin-store`  | 2.4.4 | 설정 저장 (→ [파일 생명주기 정책](file-lifecycle.md#설정-저장))              |
...
| `serde_json`            | 1.0.150 | 세션 파일 읽기·쓰기(→ [Rust 커맨드 계약](rust-commands.md#세션)) + 테스트의 직렬화 형태 검증 — serde와 버전 라인이 다름에 주의 |
...
| `trash`                 | 5.2.6   | 삭제를 휴지통 이동으로 (→ [Rust 커맨드 계약](rust-commands.md)) |
...
| 컴포넌트 카탈로그  | Storybook (`storybook`·`@storybook/react-vite`·`@storybook/addon-docs`·`@storybook/addon-a11y`·`@storybook/addon-vitest`) | 10.5.7                                            | `packages/ui` 전용 — 컴포넌트를 눈으로 보고 테마·배경·창 폭을 툴바로 바꾼다. addon-vitest가 스토리를 WebKit 렌더 테스트로 돌려 게이트에 넣는다. 텔레메트리는 끈다. 시각 회귀는 맡지 않는다 (→ [테스트 전략](testing.md))                     |
| 카탈로그 MCP       | `@storybook/addon-mcp`                                           | 0.7.0                                             | 개발 서버에만 붙는 로컬 엔드포인트(`localhost:6006/mcp`) — 에이전트가 컴포넌트 문서를 읽고 스토리 테스트를 돌린다. 정적 빌드에는 없다. 버전 라인이 Storybook 본체와 다르다   |
