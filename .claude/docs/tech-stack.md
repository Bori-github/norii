...
| 컴포넌트 카탈로그  | Storybook (`storybook`·`@storybook/react-vite`·`@storybook/addon-docs`) | 10.5.7                                            | `packages/ui` 전용 — 컴포넌트를 눈으로 보고 테마·유리를 툴바로 바꾼다. 텔레메트리는 끈다. 시각 회귀는 맡지 않는다 (→ [테스트 전략](testing.md))                     |
