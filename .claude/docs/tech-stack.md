...
| 컴포넌트 카탈로그  | Storybook (`storybook`·`@storybook/react-vite`·`@storybook/addon-docs`·`@storybook/addon-a11y`·`@storybook/addon-vitest`) | 10.5.7                                            | `packages/ui` 전용. addon-vitest가 스토리를 WebKit 렌더 테스트로 돌려 게이트에 넣는다. 텔레메트리는 끈다. 시각 회귀는 맡지 않는다 (→ [테스트 전략](testing.md))                     |
