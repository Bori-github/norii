...
**oxfmt 검증 범위**: 일곱 경로를 제외한다(`.oxfmtrc.json`이 단일 출처). ① `.claude/**` — 설계 문서 원문을 포매터가 재작성하지 않게 한다. ② `.github/**` — PR/이슈 템플릿은 플레이스홀더 구조(빈 불릿·주석)를 가진 저작 콘텐츠라 포매터가 재작성하면 깨진다. ③ `examples/**` — 앱으로 열어 눈으로 판정하는 예시 `.md`(→ [작업 규칙 — 예시 문서](../rules/project-rules.md#예시-문서-examples))라, 포매터가 공백·구조를 재작성하면 시연 재료가 변형된다. ④ `**/src-tauri/gen/**` — Tauri가 빌드마다 재생성하는 산출물이라 검증 의미가 없다(버전 관리에서도 제외). ⑤ `**/styled-system/**` — Panda가 생성하는 디자인 시스템 코드(→ [디자인 시스템](design/design-system.md#fsd-배치)). ⑥ `**/dist/**` — 빌드 산출물. ⑦ `**/shared/ipc/bindings.ts` — tauri-specta가 생성하는 IPC 바인딩(→ [기술 스택](tech-stack.md#코드-품질)의 IPC 타입 계약). 생성물 제외 기준은 oxlint(`.oxlintrc.json`)와 맞춘다.
...
주석 반복은 읽기 비용을 늘리고 한쪽만 고쳐지는 드리프트를 만든다. 사실마다 **소유자를 하나** 정하고, 소유자가 아닌 자리에는 쓰지 않는다. 문서 규칙의 단일 출처 원칙을 주석에 적용한 것이다(→ [작업 규칙 — 문서 규칙](../rules/project-rules.md#문서-규칙)).

```text
계약·정책의 내용   → .claude/docs 문서가 소유. 코드에는 "(→ 문서#절)" 링크 한 줄만 — 요약 재서술은 절대 쓰지 않는다.
                     예외: 테스트 머리의 집행 인용은 검증 대상 문구를 밝히는 것이라 재서술이 아니다(→ testing.md#테스트는-스스로를-설명한다)
판단 근거(왜)      → 테스트의 왜/보장/경계 블록이 소유(→ testing.md#테스트는-스스로를-설명한다).
                     구현 주석에 절대 반복하지 않는다. 그 사실을 다루는 테스트가 없으면 선언부가 소유한다
구현 트릭·제약     → 선언부 한 곳에만. 사용처(가드·호출부)에는 절대 주석을 달지 않는다
무엇을 하는지      → 코드가 말한다. 주석으로 절대 쓰지 않는다
```

**발견 경로·과정 메타데이터는 절대 쓰지 않는다** — "리뷰 발견 #N"·"적대적 패스"·"P1 지적" 같은 출처 표기는 그 시점의 작업 과정을 아는 사람에게만 의미가 있고 머지 순간부터 소음이다. 커밋 본문과 같은 규칙: **근거(왜 필요한가)만** 남긴다.

**마일스톤 번호·로드맵 시점은 절대 쓰지 않는다** — "M2 승인 관문"·"M5에서 추가한다"는 만든 시점(과정)을 가리킬 뿐 실체를 말하지 않고, 로드맵이 바뀌는 순간 낡는다. **실체의 이름**(정규화 승인 관문·파일 트리)으로 쓰고, 아직 없는 것의 계획은 주석이 아니라 로드맵 문서가 소유한다. 유일한 예외: 과거 실측 기록("실측: 246KB 렌더 202ms")은 측정 시점·환경이 값의 맥락이므로 유지한다.

도구가 강제하지 못하는 규칙이므로 리뷰(사람·에이전트)가 지킨다.
...
lefthook으로 커밋 전에 게이트를 자동 실행한다. 전체 `check`는 실행이 느릴 수 있으므로, 커밋 훅에서는 **변경된 파일에만 oxlint/oxfmt**를 돌리고, **전체 게이트는 CI가 강제한다**(`.github/workflows/ci.yml` — PR·main 푸시마다 mac 러너에서 `mise run check`). 2단 구조의 의도: 1단(훅)은 빠른 피드백이고, 2단(CI)은 로컬에서 게이트가 실수·우회로 빠져도 **병합 경계에서 기계적으로 잡는** 최종 방어다. 커밋 전 `check` 통과(→ [작업 규칙](../rules/project-rules.md#품질-게이트))는 여전히 원칙이지만, 그 준수가 사람의 손 순서에만 의존하지 않게 됐다.
