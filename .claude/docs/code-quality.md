...
oxfmt는 현재 **0.x(베타)** 다. 스택에서 유일하게 1.0 미만인 도구다. 다만 Vue.js·Turborepo·Sentry 등 광범위하게 채택됐고 활발히 개선 중이다. **리스크 관리**:
...
- **유닛·순수 로직**: Vitest. `packages/markdown`(파서 파이프라인)·`shared/lib`·`entities/*/model` 같은 순수 로직을 우선 덮는다.
- **컴포넌트**: `@testing-library/react` + `happy-dom` 환경. 사용자 관점(역할·텍스트)으로 검증한다.
- **커버리지**: `@vitest/coverage-v8`로 측정. 초기에는 **하드 임계값을 강제하지 않고** 리포트만 본다(스캐폴드 단계에서 임계값은 방해가 됨). 코어 로직이 안정되면 임계값을 도입한다(열린 결정 — [실제 구현 계획](implementation-plan.md#열린-결정-open-decisions)).
- **원칙**: 커버리지 숫자를 목표로 삼지 않는다. "이 테스트가 실패하면 어떤 사용자 경험이 깨지는가"를 기준으로 쓴다.

## lefthook (커밋 전 자동화)

lefthook으로 커밋 전에 게이트를 자동 실행한다. 전체 `check`는 무겁게 느껴질 수 있으므로, 커밋 훅에서는 **변경된 파일에만 oxlint/oxfmt**를 돌리고, 전체 게이트는 CI(push/PR)에서 강제하는 2단 구조를 권한다.
...
E2E는 아직 확정하지 않는다. Tauri의 E2E는 **WebDriver(tauri-driver + WebdriverIO)** 기반인데, **macOS의 WKWebView는 WebDriver 지원이 제한적**이라 mac 우선 개발과 상충한다. 초기에는:

- 유닛·컴포넌트는 Vitest로 충분히 덮는다.
- 앱 수준 검증은 수동 체크리스트(한글 IME·대용량 문서 스크롤 등, → [에디터 전략](editor-strategy.md))로 보완하고, 자동화 불가 사유를 기록한다(→ [작업 규칙](../rules/project-rules.md)).
- 크로스플랫폼(Windows/Linux) 확장 시 WebDriver E2E를 재검토한다(→ [플랫폼 전략](platform-strategy.md)).
