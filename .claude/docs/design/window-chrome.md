...
> **현재 코드는 이 계약을 아직 구현하지 않았다.** 창 설정 네 키가 모두 부재한 상태이며, 적용은 **M5(다듬기)** 에 배정되어 있다(→ [실제 구현 계획](../implementation-plan.md#마일스톤)). 문서가 코드보다 앞서 있는 정상 상태다.

**왜** 이렇게 하는지는 [DESIGN.md](../../../DESIGN.md#결정-2--유리는-os가-만든다-css가-아니라)가, App Store를 포기하는 배포 판단은 [플랫폼 전략](../platform-strategy.md#배포-경로--app-store는-비목표)이 소유한다. 표면의 역할·재질 규칙은 [DESIGN.md 결정 1](../../../DESIGN.md#결정-1--표면은-역할과-재질-두-축으로-정한다)이 소유한다. 이 문서는 **무엇을 어떤 값으로** 만 다룬다.
...
신호등 인셋   좌측 상단 신호등 아래로 콘텐트가 들어가지 않게 탭바 좌측에 인셋을 둔다.
             인셋 폭은 OS 버전에 따라 다르므로 하드코딩하지 않고 실측으로 정한다.
```

- `radius`는 지정하지 않는다. 창 모서리는 OS가 그린다. **단, 투명 창의 라이브 리사이즈·코너·그림자 아티팩트는 미검증**이며 실기 확인 항목이다.
...
**선행 조건**: 캔버스를 투명으로 바꾸는 변경은 **편집면과 CM6 테마에 불투명 배경을 명시적으로 칠한 뒤에만** 들어간다. 현재 편집면은 배경을 칠하지 않고 캔버스를 비쳐 쓰므로, 순서를 지키지 않으면 본문이 바탕화면 위에 뜬다(→ [DESIGN.md](../../../DESIGN.md#종이는-명시적으로-칠한다-투명-캔버스의-선행-조건)).
...
자동  ① 크롬 틴트 대비 기준 — 토큰 값만으로 계산되는 순수 함수라 유닛 테스트가 막는다
        (→ DESIGN.md 접근성 기준 · design-system.md 대비 게이트)
