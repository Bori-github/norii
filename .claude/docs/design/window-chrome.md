...
**웹 쪽은 이만큼 위를 비운다.** 탭바가 `_glass`에서 `padding-top: 28px`를 두어 탭이 띠 아래에서 시작한다. 침범하면 탭을 눌러도 클릭이 띠에 먹혀 창이 끌린다. 두 값은 같아야 하고, 위 상수가 단일 출처다. 비운 그 자리에 **앱 이름만** 그린다(클릭을 받지 않는 글자).
