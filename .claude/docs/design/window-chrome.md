...
**웹 쪽은 이만큼 위를 비운다.** 탭바가 `_glass`에서 `padding-top: 36px`를 두어 탭이 띠 아래에서 시작한다. 침범하면 탭을 눌러도 클릭이 띠에 먹혀 창이 끌린다. 두 값은 같아야 하고, 위 상수가 단일 출처다.

**비운 자리에는 앱 이름과 클릭 통과 영역 하나가 선다.** 앱 이름은 클릭을 받지 않는 글자다(`pointer-events: none`). 그 밖에 띠 안에서 눌리는 컨트롤을 두려면 **클릭 통과 영역**을 둬야 한다 — 띠의 `hitTest:`가 그 사각형 안에서 `nil`을 반환해 클릭이 웹뷰로 내려간다. 영역은 하나뿐이고 사이드바 토글이 쓴다. 그 자리는 드래그 손잡이가 아니다.

클릭 통과 영역의 x는 표준 창 버튼을 피한다 — 그 버튼은 창의 theme frame이 그리므로 띠(contentView 안)보다 위에 있어 가려지지 않지만, 겹쳐 두면 토글이 버튼 아래로 들어간다. 웹 쪽은 `_glass`에서만 토글을 이 좌표로 올리고, 유리가 꺼지면 띠가 없으므로 탭바 흐름에 그대로 둔다.
