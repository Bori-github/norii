...
design/window-chrome.md          창 설정의 실제 값 — transparent · windowEffects · 폴백
...
text.muted    흐린 글자. 종이 위에서만 — 크롬에는 금지(→ [결정 0004](decisions/0004-lime-is-the-only-accent.md)).
accent        액센트. 종이 위에서만 — 크롬에는 금지.
              테마가 단계를 가른다(다크=형광 라임 · 라이트=올리브). 컴포넌트는 이 토큰 하나만 본다.
```

두 금지는 취향이 아니라 [대비 게이트](#대비-게이트)의 계산 결과다. 브랜드 보라는 앱 UI에서 쓰지 않으므로 **시맨틱 토큰을 갖지 않는다** — 원시 토큰으로만 존재한다.
