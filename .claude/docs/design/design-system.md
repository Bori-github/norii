...
디자인 문서는 셋으로 나뉜다. 같은 사실을 복제하지 않는다.

```text
/DESIGN.md                          무엇을·왜 — 아이덴티티 · 표면 계층 · 색/타이포 원칙 · 접근성 기준
.claude/docs/design/design-system.md  어떻게 — Panda 토큰 계층 · recipe · 테마 조건 · FSD 배치 (이 문서)
.claude/docs/design/window-chrome.md  창 표면의 실제 설정값 — transparent · windowEffects · 폴백
apps/desktop/panda.config.ts        값 — 팔레트·스케일의 실제 숫자. 문서가 아니라 코드가 소유한다.
```

디자인 관련 문서는 `.claude/docs/design/`에 모은다. 루트 `DESIGN.md`만 예외로 남긴다 — 저장소를 여는 사람이 가장 먼저 보게 될 헌장이기 때문이다.
...
[DESIGN.md 결정 1](../../../DESIGN.md#결정-1--표면은-역할과-재질-두-축으로-정한다)의 역할·재질 규칙은 **배경 토큰의 이름으로** 코드에 나타난다. 컴포넌트는 "얼마나 투명한가"를 몰라도 되고, **자기가 무엇인지만** 고르면 된다. 어느 표면이 어느 토큰을 쓰는지는 DESIGN.md가 단일 출처다 — 여기서는 토큰 이름만 정의한다.

```text
bg.canvas    창 바닥. 유리가 켜지면 투명, 아니면 불투명. 갈라지는 유일한 토큰.
bg.chrome    도구 표면 — 유리 위에 얹는 틴트. 알파는 대비 게이트가 하한을 정한다.
...
- **활성 탭은 `bg.paper`다.** 크롬 안에 있지만 아래 편집면과 이어진 한 장의 종이로 읽힌다(→ DESIGN.md 결정 1).
- 편집면은 **배경을 명시적으로 칠한다.** 캔버스를 비쳐 쓰면 유리 도입 시 본문이 뚫린다.

## 대비 게이트

DESIGN.md의 [접근성 기준](../../../DESIGN.md#접근성-기준)은 **토큰 값만으로 계산되는 순수 함수**라 자동 검증한다. 유리 뒤 바탕화면은 통제할 수 없지만, 크롬 틴트의 알파 α가 정해지면 합성 결과가 구간에 갇힌다.
...
> **아직 구현되지 않았다.** 현재 `packages/editor`에는 CM6 테마 객체가 없고 색은 CM6 내장 팔레트가 그린다 — 검색 패널 배경(`#f5f5f5`)이 대표적으로 앱 토큰 밖에 있다. 테마 주입은 M5에서 하며, 그때 `packages/editor`의 공개 시그니처에 테마 파라미터가 생긴다.
