...
**단, 이 결정은 비대칭이다.** 유리를 나중에 포기하는 것은 가능하지만, App Store로 **복귀**하는 것은 그렇지 않다 — 직접 배포를 전제로 굳어지는 서명·공증·자동 업데이트 선택을 되돌려야 한다. 되돌리기 비용의 전체 목록은 [유리](design/decisions/glass.md#되돌리기-비용)에 있다.
...
아키텍처:        x86_64·arm64를 한 파일에 담아 배포(--target universal-apple-darwin).
번들 크기:       측정·유지(→ #번들-크기-측정).
```

배포 구현은 [실제 구현 계획](implementation-plan.md)의 배포 단계에서 다룬다. 번들 크기는 첫 단계부터 빌드마다 측정해 추세를 지킨다.

## 번들 크기 측정

`mise run bundle-size`(`scripts/bundle-size.mjs`, 의존성 없는 Node)로 측정한다. `check` 게이트에는 넣지 않는다 — 빌드 산출물이 있어야 하므로 프론트(`pnpm --filter desktop build`) · 앱(`tauri build --bundles app`) 뒤에 돈다. (`mise run build`는 풀 tauri 릴리스 빌드이며, `bundle.active:false`라 `.app`을 만들려면 `--bundles app`이 필요하다 — 상시 번들링은 배포 단계(→ [실제 구현 계획](implementation-plan.md#마일스톤)).)

**무엇을 재나** — 두 층:

```text
앱 번들(.app/.dmg)   예산의 대상. src-tauri/target 아래 release/bundle 산출물 전부.
...
앱 번들이 없으면(빌드 전) 안내만 하고 통과한다. macOS 앱 번들 측정은 배포 단계에서 CI에 붙인다(→ [실제 구현 계획](implementation-plan.md#마일스톤)).
...
2026-07-14 (M4 프리뷰 확장: 각주·수식·다이어그램, macOS)
  앱 번들      11.09 MB → 12.75 MB  (+1.66 MB)
  프론트 dist   0.89 MB →  5.15 MB  (+4.26 MB — 앱에서는 1/3 수준으로 줄어 실림)

2026-08-22 (M5~M10 · pkg-ui 반영분, macOS)
  앱 번들(1아키텍처)  12.75 MB → 14.12 MB  (+1.37 MB)
  앱 번들(2아키텍처)              23.15 MB  (릴리스 프로파일 strip 적용 시)
  프론트 dist          5.15 MB →  5.38 MB
```
