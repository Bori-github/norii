...
**단, 이 결정은 비대칭이다.** 유리를 나중에 포기하는 것은 가능하지만, App Store로 **복귀**하는 것은 그렇지 않다 — 직접 배포를 전제로 굳어지는 서명·공증·자동 업데이트 선택을 되돌려야 한다. 되돌리기 비용의 전체 목록은 [결정 0003](design/decisions/0003-opaque-fallback-outside-macos.md#되돌리기-비용-정직한-판)에 있다.
