...
pnpm --filter desktop exec playwright install webkit
                 # 브라우저 모드 테스트용 WebKit 바이너리 (1회, CI와 동일 명령, → testing.md)
```
...
pnpm --filter desktop build   # vite 빌드 → dist 생성
mise run bundle-size          # dist 측정 · 앱 번들 있으면 예산과 비교 (→ platform-strategy.md)

# 실제 .app 크기(예산의 대상)를 재려면 번들링을 켜서 릴리스 빌드한다:
...
`mise run check`는 위 검증 태스크(`fmt-check`·`lint`·`fsd-lint`·`typecheck`·`test`·`rust-fmt-check`·`clippy`·`rust-test`·`docs-drift`)를 모두 실행한다. 포맷을 **수정**하는 건 `mise run fmt`뿐이고, 게이트는 검증만 한다.
...
`mise run check` 게이트에 편입돼 있다. 구현은 `scripts/docs-drift.mjs`(의존성 없는 Node 스크립트)다. 문서-코드 간극을 성실함이 아니라 **게이트가 잡는다**. 대조 대상은 [작업 규칙](../rules/project-rules.md)의 계약 문서 중 기계 대조 가능한 표면이다:
