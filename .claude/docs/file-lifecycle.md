...
| 항목 | 정책 |
|---|---|
| Dirty 추적 | CM6 `docChanged`로 감지. 자동 저장 전 "저장 대기"를 탭의 상태 점으로 표시(→ [문서 모델](document-model.md) · [컬러 팔레트](design/decisions/color-palette.md#탭-상태-점)). |
| 자동 저장 | **기본 5초** — 첫 변경 뒤 그 간격에 저장. `Cmd+S`는 즉시 저장(→ [자동 저장](#자동-저장)). |
...
- **간격은 설정 화면에서 고른다** — 끄기 · 5초 · 10초 · 30초 · 1분. 기본은 5초이고, 값은 `features/save-file/config.ts`가 소유한다.
- **탭에 저장 대기가 생긴 시점부터 그 간격이 지나면 저장한다.** 저장이 나간 뒤 다시 고치면 그때부터 다시 잰다. **타이핑이 이어져도 예약을 미루지 않는다** — 미루면 긴 간격에서 계속 입력하는 동안 저장이 한 번도 나가지 않는다. "편집본과 디스크가 다른" 구간의 길이는 고른 간격이 상한이다.
- **끄기를 고르면** 저장은 `Cmd+S` 수동이고, 끄기 전에 잡혀 있던 예약도 나가지 않는다. 종료·탭 닫기가 무엇을 하는지는 [종료 방어](#종료-방어)가 소유한다.
- `Cmd+S`는 예약을 기다리지 않는 **즉시 저장**이고, 남아 있던 예약을 취소한다(→ [에디터 전략 — 단축키 계약](editor-strategy.md#단축키-계약)).
- **Untitled 탭(`filePath=null`)은 자동 저장 대상이 아니다** — 저장할 경로가 없다. 첫 저장은 수동(`Cmd+S` → 다이얼로그)이며, [종료 방어](#종료-방어)에서 다이얼로그가 뜨는 유일한 평상 케이스다.
- **정규화 승인 — 저장이 원본 바이트를 재작성하는 탭은 자동 저장이 꺼진 채 열린다.** 현재 두 경우다: ① 비UTF-8로 감지된 파일(`sourceEncoding ≠ 'utf-8'`, UTF-16 포함) — 저장 시 UTF-8로 변환됨, ② 개행이 재작성되는 파일(혼합 EOL·CR-only, `eolMixed`) — 저장 시 판정 EOL로 통일됨. 이런 탭은 배너가 "저장 시 무엇이 바뀌는지"를 알리고, **사용자가 배너에서 승인하거나 첫 수동 저장(`Cmd+S`)을 하면 정규화가 승인된다** — 전역 자동 저장 설정이 끄기가 아닌 경우 그때부터 이 탭도 자동 저장 대상이 된다(승인은 탭 상태이고, 자동 저장 활성은 전역 설정이다 — 승인이 꺼둔 설정을 뒤집지 않는다). 승인 전까지 자동 저장은 그 파일을 건드리지 않는다 — [인코딩 정책](#인코딩-정책)의 "저장 전까지 원본 불변" 안전망을 자동 저장에서도 지키는 규칙이다. 승인 후 첫 저장이 성공하면 탭 메타를 갱신한다(`sourceEncoding='utf-8'`·`eolMixed=false`, 배너 해제) — 변환은 1회로 끝난다. **일반 원칙**: 사용자가 입력하지 않은 바이트 변경을 저장이 수행하게 되는 기능이 향후 추가되면(예: 후행 공백 제거) 그것도 이 승인 대상에 편입한다.
- **충돌 시 일시 중지**: 자동 저장이 `AppError::Conflict`를 받으면 해당 탭의 자동 저장을 일시 중지하고 충돌 배너를 띄운다. 사용자가 해소(디스크/편집 선택)하면 재개한다 — 자동 저장이 돌 때마다 배너가 반복되는 것을 막는다.
...
sequenceDiagram
    ...
    Note over S: 저장 대기가 생길 때 예약<br/>(이어지는 타이핑은 예약을 미루지 않는다)
    S->>Q: 간격 만료 → 플러시 요청
...
stateDiagram-v2
    ...
    [*] --> clean
    clean --> dirty: 편집
    dirty --> saving: 예약 만료 · Cmd+S · 닫기 플러시
    saving --> clean: 성공
    saving --> dirty: 성공했지만 저장 중 또 편집됨
    saving --> conflict: 해시 불일치
    saving --> dirty: 저장 실패(알림)
...
- **자동 저장이 꺼져 있으면 플러시하지 않는다** — dirty 탭 전체가 다이얼로그 대상이고, 탭 하나를 닫을 때도 같다(→ [문서 모델](document-model.md#다중-탭-규칙)). 플러시하면 `Cmd+S`를 누른 적 없는 편집이 디스크에 쓰인다.
...
**비활성 탭의 충돌 표시**: 충돌 배너는 활성 탭에 뜬다. 비활성 탭에서 충돌이 나면 그 탭의 **상태 점이 위험색으로 바뀌고 제목도 같이 칠해진다**(→ [컬러 팔레트](design/decisions/color-palette.md#탭-상태-점)). 사용자가 그 탭으로 전환하면 충돌 배너가 뜬다. 표시는 시각 신호만 더하고 작업 흐름을 끊지 않는다(자동 탭 전환은 하지 않는다). **파일 삭제(file-removed)도 같은 표시를 쓰되 접근성 이름은 삭제 전용**이다 — 탭을 열었을 때의 안내(충돌 선택 vs 재생성 저장)가 다르므로 이름에서부터 구별한다.
...
편집본과 디스크가 다른 구간은 고른 간격을 넘지 않는다. 충돌 안내는 그 구간 안에 외부 도구가 같은 파일을 고쳤을 때만 뜬다. 간격이 길수록 그 구간도 길어진다.
...
세션 복원 같은 UI 상태는 `.md` 본문이 아니라 앱 config/사이드카에 저장한다. 접힘 상태는 저장하지 않는다. `.md`에 메타데이터를 섞으면 파일이 지저분해지고 다른 에디터 호환이 깨진다. 이 경계는 [비목표](../rules/non-goals.md#접힘-상태-영속화의-경계)를 단일 출처로 둔다.

### 설정 저장

테마·유리 불투명도·흐림 반경·뷰 모드·사이드바 접힘·자동 저장 간격은 `plugin-store`의 JSON 하나에 저장한다. 파일 이름·키·디바운스 간격은 `shared/config`가 소유한다.

```text
쓰기        값이 바뀔 때 디바운스해 쓴다
닫기        디바운스가 남아 있으면 창을 닫기 전에 쓴다(→ 종료 방어)
읽기 실패    기본값으로 계속 간다
모르는 값    무시하고 기본값을 쓴다 — 저장 파일은 손으로 고칠 수 있다
범위 밖 값   스토어의 자르기를 그대로 지난다(두 번째 규칙을 만들지 않는다)
```

읽는 시점과 창이 보이는 순서는 [창 표면 계약](design/window-chrome.md#부팅-순서--창은-언제-보이는가)이 소유한다.
