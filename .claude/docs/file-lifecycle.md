...
- **끄기는 설정 화면에서 고른다.** off면 저장은 `Cmd+S` 수동이고, [종료 방어](#종료-방어)는 플러시 대신 dirty 탭 전체에 확인 다이얼로그를 띄운다. 끄기 전에 걸린 예약도 나가지 않는다 — 끈 뒤에 나가는 저장은 설정을 거짓으로 만든다.
...
- **자동 저장이 꺼져 있으면 플러시하지 않는다** — dirty 탭 전체가 다이얼로그 대상이다. 탭 하나를 닫을 때도 같다(→ [문서 모델](document-model.md#다중-탭-규칙)). 끈 사용자에게 종료가 저장을 대신하면 그 설정이 거짓이 된다.
...
테마·유리 불투명도·흐림 반경·뷰 모드·사이드바 접힘·자동 저장 켜짐은 `plugin-store`의 JSON 하나에 저장한다. 파일 이름·키·디바운스 간격은 `shared/config`가 소유한다.
