...
**alt 텍스트는 브라우저가 아니라 프리뷰가 표시한다.** WKWebView는 실패한 이미지 자리에 물음표가 든 상자를 그리고 `alt`는 글자로 표시하지 않는다. `img`는 대체 요소라 `::after`도 화면에 나오지 않는다(실앱 실측 — 계산 스타일은 잡힌다). 그래서 실패한 `img`에 CSS로 `display: none`을 걸고, 그 뒤에 alt 텍스트를 담은 `<span>`을 넣는다.
