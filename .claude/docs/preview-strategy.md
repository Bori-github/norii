...
세 기능(각주·수식·다이어그램)이 실제로 얼마를 먹는지 `.app`을 빌드해 쟀다(2026-07-14, macOS).

```text
앱 번들     11.09 MB → 12.75 MB  (+1.66 MB)
프론트 dist  0.89 MB →  5.15 MB  (+4.26 MB)
```

**dist 증가분이 앱에서 1/3로 줄어든다** — Tauri가 웹 자산을 압축해 바이너리에 넣기 때문이다. 따라서 예산 판정의 기준은 dist가 아니라 `.app`이며, 예산 값과 측정 방법은 [플랫폼 전략 — 번들 크기 측정](platform-strategy.md#번들-크기-측정)이 소유한다. mermaid를 포함해도 예산 안이므로 범위를 줄이지 않는다.
