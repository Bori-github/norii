...
`![](./사진.png)`과 원시 HTML의 `<img>`를 프리뷰에 띄운다. 로컬 파일은 문서가 있는 폴더를 기준으로 찾는다.

### 경로 해석

```text
스킴이 있다 (https: · data: …)   그대로 둔다
문서 경로가 없다 (Untitled)       그대로 둔다 — 기준 폴더가 없다
그 밖 (상대·절대 경로)            문서 폴더 기준의 절대 경로로 바꿔 asset URL로 싣는다
```

- 로컬 파일은 **Tauri asset 프로토콜**로 싣는다 — 읽을 수 있는 경로는 [Rust 커맨드 계약 — 권한](rust-commands.md#권한-capabilities)이 정한다.
...
`https:` 이미지를 싣고 `http:`는 싣지 않는다. 허용 스킴과 그 대가는 [보안 — CSP](security.md#1-tauri-csp-content-security-policy)를 단일 출처로 둔다.

### 못 찾은 이미지

로드에 실패한 `img`에 표시를 남기고, 그 자리에 **흐린 글자로 alt 텍스트**가 남는다. 배너를 띄우지 않는다 — 수식·다이어그램 실패와 같은 원칙이다.
