...
**링크 정책**: 프리뷰 속 링크 클릭은 **웹뷰 내비게이션을 항상 차단**하고(앱이 문서 속 URL로 통째로 이동하면 앱 UI가 사라진다), 안전한 링크만 **OS 기본 브라우저**로 넘긴다(`plugin-opener`).

```text
http:  https:  mailto:   → OS 기본 브라우저로 연다
file:  그 외 모든 스킴    → 아무 동작도 하지 않는다(거부)
상대 경로(문서 내 앵커)   → 아무 동작도 하지 않는다 — 문서 내 이동은 M6 이후 과제
```

스킴 허용목록이 방어의 핵심이다 — 문서는 신뢰하지 않는 입력이라, `file:`(로컬 파일 열기)이나 커스텀 스킴(외부 앱 실행·딥링크)을 그대로 넘기면 악성 `.md` 한 줄이 클릭 한 번으로 앱을 실행시킬 수 있다(→ [보안 — 외부 링크](security.md)). 판정은 URL 파싱으로 하고 문자열 접두사 비교에 기대지 않는다.
