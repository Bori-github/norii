...
**링크 정책**: 프리뷰 속 링크 클릭은 웹뷰 내비게이션을 차단한다 — 앱이 문서 속 링크로 통째로 이동하는 것을 막는다. OS 브라우저로 열기(opener 플러그인 = 새 capability)는 [열린 결정](implementation-plan.md#열린-결정-open-decisions)이다.
