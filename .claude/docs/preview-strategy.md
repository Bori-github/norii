...
- 모드는 세션 한정 전역 상태다 — 저장 여부는 탭 세션 복원과 함께 정한다(→ [문서 모델](document-model.md#세션-복원) · [실제 구현 계획](implementation-plan.md)).
...
- **색은 경계선만 진다.** `NOTE`·`TIP`·`WARNING`·`CAUTION`이 각각 정보·성공·경고·위험 상태색을 왼쪽 경계선에 쓴다. `IMPORTANT`는 상태가 아니라 강조라서 액센트를 쓴다. **라벨에는 색을 쓰지 않는다** — 라벨은 글자라 AA(4.5:1)가 필요한데 상태색은 표시 전용이라 통과하지 못한다(→ [컬러 팔레트](design/decisions/color-palette.md)).

  구별은 **경계선 색 · 아이콘 · 라벨** 셋이 함께 맡는다. 색 하나에 기대지 않으므로 색을 구별하지 못하는 사용자도 종류를 읽을 수 있다.
