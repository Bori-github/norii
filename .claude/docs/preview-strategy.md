...
- **프리뷰가 다시 렌더될 때마다 버튼은 사라진다** — 내용을 `innerHTML`로 통째로 교체하기 때문이다(→ [프리뷰 DOM은 React가 소유하지 않는다](#프리뷰-dom은-react가-소유하지-않는다)). 다이어그램 SVG와 같은 경로로 **삽입 뒤 다시 붙인다.**
- **클릭 처리는 패널 하나에 이벤트 위임**으로 둔다 — 코드 블록마다 리스너를 달면 블록 수만큼 리스너가 생기고 매 렌더마다 다시 등록된다.
