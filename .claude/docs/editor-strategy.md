...
| 접기/펼치기 | `Cmd+Alt+[` / `Cmd+Alt+]` | `Ctrl+Shift+[` / `Ctrl+Shift+]` | CM6 기본 `foldKeymap` — **M5 예정**(폴딩과 함께 등록, 아직 미등록) |
| undo/redo | `Cmd+Z` / `Cmd+Shift+Z` | `Ctrl+Z` / `Ctrl+Y` | CM6 기본 `historyKeymap` |

CM6가 기본 제공하는 키맵(`defaultKeymap`의 표준 편집 조작 · history · search, 폴딩 도입 시 fold)은 그대로 채택하고, 앱 전역 동작만 커스텀 키맵으로 추가한다. 표에 없는 단축키를 추가할 때는 이 표를 먼저 갱신한다. 저장(`Cmd+S`)은 자동 저장의 디바운스를 기다리지 않는 **즉시 저장**이다(→ [파일 생명주기 정책 — 자동 저장](file-lifecycle.md#자동-저장)).
