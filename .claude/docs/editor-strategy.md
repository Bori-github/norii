...
버전은 [기술 스택](tech-stack.md)을 단일 출처로 둔다. 폴딩용 별도 파서는 필요 없다 — `@codemirror/lang-markdown`이 만드는 구문 트리를 `@codemirror/language`의 fold 기능으로 그대로 이용한다(→ [기술 스택](tech-stack.md#왜-에디터는-codemirror-6인가)).
...
```text
1단계: 헤딩 접기       #, ## 아래 내용을 접음. CM6 기본 폴딩으로 거의 공짜.
...
- **초기: 저장하지 않는다.** 열 때마다 펼친 상태로 시작한다. `.md` 순수성을 100% 보장한다.
...
- CM6 증분 파서·가상화를 신뢰한다. 대용량 문서(수만 줄) 스크롤 테스트를 QA 체크리스트에 포함한다.

## 한글 IME

소스 뷰는 WYSIWYG보다 IME에 안전하지만, 커스텀 키맵·자동완성이 조합(composition) 이벤트와 충돌할 수 있다.

- **초기부터 한글 입력 테스트를 필수 수동 체크리스트에 포함**한다(→ [작업 규칙](../rules/project-rules.md)).
