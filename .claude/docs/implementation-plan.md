...
기존 팔레트 AA 위반 현재 라이트 테마의 muted·액센트 라벨이 4.5:1 미달 — 확정 팔레트를 panda.config.ts에 반영할 때 해소 (→ design/design-system.md#대비-게이트)
폰트 스택 값      UI sans · 에디터 고정폭의 실제 스택 — 한글 폴백을 명시해 렌더를 결정적으로 만든다 (→ design/design-system.md#테마-라이트다크)
...
한글 고정폭 폰트  표 정렬까지 지키려면 듀오스페이스 한글 폰트 번들(수 MB) 필요 — 15MB 예산과 함께 판단 (→ design/design-system.md#테마-라이트다크)
