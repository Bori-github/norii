...
| M6. UI 다듬기 | 예정 | **시각 완성도를 책임지는 단계.** 타이포 스케일 토큰 확정 · 프리뷰 타이포/상호작용 · 탭 ⚠ 배지 색 · **설정 화면**(테마 선택 · 유리 불투명도/흐림 반경) · 설정 영속화(자동 저장 토글 포함) · 세션 복원 · **한글 IME QA**(E2E) · 사이드바 다듬기(빈 폴더 "비어 있음" 표시 · ARIA 트리 시맨틱과 키보드 탐색 — 반쪽 ARIA는 없느니만 못하므로 한 세트로) · 대형 폴더 실측(수천 항목 스크롤 — 트리 가상화 도입 판단, 성능 규칙의 "측정 없이 주장하지 않는다") | 실사용 가능한 시각 완성도 |
