...
| M10. 문서 속 이미지 | 예정 | **문서 옆에 둔 이미지와 원격 이미지를 프리뷰에 띄운다.** 경로 해석·sanitize 경계·실패 표시는 [프리뷰 전략 — 이미지](preview-strategy.md#이미지)가, 로컬 이미지가 지나는 통로의 경로 제한은 [Rust 커맨드 계약 — 권한](rust-commands.md#권한-capabilities)이 확정했다 | 폴더를 열지 않아도 문서 옆 이미지가 프리뷰에 뜨고, 없는 이미지는 그 자리에 알린다 |
