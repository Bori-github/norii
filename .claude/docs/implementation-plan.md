...
| M10. 성능·규모 | 예정 | 대형 폴더 트리 가상화(현재 미도입 → [문서 모델](document-model.md#파일-트리-사이드바)) · 프리뷰 렌더 후속(→ [프리뷰 전략](preview-strategy.md#디바운스)) | 파일이 수천 개인 폴더와 긴 문서에서도 즉시 반응한다 |
| M11. 배포(mac) | 예정 | 코드 서명·공증 · 자동 업데이트 · 번들 크기 목표 확정 | 배포 가능한 서명된 mac 빌드 |
