...
| M6. UI 다듬기 | 예정 | **시각 완성도를 책임지는 단계.** 타이포 스케일 토큰 확정 · 프리뷰 타이포/상호작용 · 탭 ⚠ 배지 색 · **설정 화면**(테마 선택 · 유리 불투명도 · 흐림 반경) · 설정 영속화(자동 저장 토글 포함) · 세션 복원 · **한글 IME QA**(→ [한글 IME](korean-ime.md)) · 사이드바 다듬기(ARIA 트리 시맨틱과 키보드 탐색 — 반쪽 ARIA는 없느니만 못하므로 한 세트로) · **사이드바 파일 조작**(새 파일/폴더 · 이름 변경 · 휴지통 삭제 → [Rust 커맨드 계약](rust-commands.md)) · 대형 폴더 성능(가상화는 현재 미도입 → [문서 모델](document-model.md#파일-트리-사이드바)) | 실사용 가능한 시각 완성도 |
...
>
> 테마(라이트/다크 · CM6 테마 토큰 통합)와 **창 유리**(투명 창 + 창 뒤 흐림 → [창 표면 계약](design/window-chrome.md))는 원래 다듬기 항목이었으나 별도 트랙에서 구현했다. 그 조각들 — **설정 화면**(테마 선택 · 유리 값 조절)과 영속화 — 은 M6이 맡는다.
