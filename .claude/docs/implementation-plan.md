...
| M5. 다듬기 | 리스트 접기 · 테마(라이트/다크 · CM6 테마 토큰 통합) · **창 유리**(투명 창 + vibrancy → [창 표면 계약](design/window-chrome.md)) · 설정 영속화(자동 저장 토글 포함) · 세션 복원 · **한글 IME QA**(E2E) | 실사용 가능한 수준 |
...
M5  → editor-strategy.md · file-lifecycle.md · design/design-system.md · design/window-chrome.md · document-model.md
...
팔레트·타이포 값  액센트 1색 + 편향된 무채색 스케일 · 폰트 스택의 실제 값 — 시안 승인 후 확정. 대비 게이트가 합격선 (→ DESIGN.md)
기존 팔레트 AA 위반 현재 라이트 테마의 muted 텍스트·액센트 라벨이 4.5:1 미달(실측) — 팔레트 확정 시 함께 수정 (→ DESIGN.md#접근성-기준)
유리 재질 선택    macOS windowEffects 재질(sidebar / underWindowBackground / hudWindow) — 실기 실측으로 확정 (→ design/window-chrome.md)
떠 있는 면 흐림   투명 창에서 backdrop-filter가 동작하지 않는다는 보고(tauri#6876·#12804) — 불투명으로 시작하고 실측 후 채택 여부 결정 (→ DESIGN.md#결정-2--유리는-os가-만든다-css가-아니라)
투명 창 아티팩트  라이브 리사이즈·코너·그림자가 투명 창에서 깨지지 않는지 실기 확인 (→ design/window-chrome.md#계약--tauriconfjson)
신호등 인셋 폭    Overlay 타이틀바에서 좌측 인셋 폭 — OS 버전차가 있어 실측으로 확정 (→ design/window-chrome.md)
한글 고정폭 폰트  표 정렬까지 지키려면 듀오스페이스 한글 폰트 번들(수 MB) 필요 — 15MB 예산과 함께 판단 (→ DESIGN.md#타이포-원칙)
...
경로 정규화 비교   스코프 허용 루트 비교가 바이트 단위 — 대소문자·NFC/NFD가 다른 입구(M4 트리·M5 세션 복원) 추가 전 비교 전략 확정. 심링크 별칭(/tmp↔/private/tmp)으로 같은 파일이 두 탭으로 열리는 문제도 이 결정에 포함 (→ rust-commands.md)
...
watch 견고성 후속  M2 리뷰 지적 4건 — 감시 교체 순서(새 감시 생성 후 교체)·경로별 부분 실패 허용·이벤트 코얼레싱(디바운스)·재선언 직렬화 + removed 이벤트 큐 지연. 분리 단위로 보고 후 진행 (→ rust-commands.md watch_paths)
