...
| M7. 상태 유지 | 예정 | 세션 복원(탭 목록 · 활성 탭 · 루트 폴더 → [문서 모델](document-model.md#세션-복원)) · 자동 저장 토글(→ [파일 생명주기](file-lifecycle.md#자동-저장)) | 껐다 켜면 열려 있던 탭과 폴더가 돌아온다 · 자동 저장을 끌 수 있다 |
| M8. 시각 완성도 | 예정 | **전체 UI 쇄신** — 창 모서리·배경·크롬(타이틀바·사이드바·탭바·상태바)을 한 벌로 다시 맞춘다 · 타이포·간격 스케일 확정 · 프리뷰 타이포/상호작용 · 스크롤바 전역 스타일 | 화면 전체가 한 벌로 읽히고, 크기와 간격이 한 스케일에서 나온다 |
| M9. 성능·규모 | 예정 | 대형 폴더 트리 가상화(현재 미도입 → [문서 모델](document-model.md#파일-트리-사이드바)) · 프리뷰 렌더 후속(→ [프리뷰 전략](preview-strategy.md#디바운스)) | 파일이 수천 개인 폴더와 긴 문서에서도 즉시 반응한다 |
| M10. 배포(mac) | 예정 | 코드 서명·공증 · 자동 업데이트 · 번들 크기 목표 확정 | 배포 가능한 서명된 mac 빌드 |
...
- M9 → [문서 모델 — 파일 트리](document-model.md#파일-트리-사이드바) · [프리뷰 전략 — 디바운스](preview-strategy.md#디바운스)
- M10 → [플랫폼 전략](platform-strategy.md#배포-경로--app-store는-비목표)
...
- **탭 세션 복원** — 커서 위치까지 복원할지 · 에디터/프리뷰 스크롤도 탭별 복원할지(VS Code 방식 — 현재는 화면 1개 공유, 전환 시 맨 위) · 뷰 모드도 복원 대상인지 (→ [문서 모델](document-model.md) · [프리뷰 전략](preview-strategy.md))
