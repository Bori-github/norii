...
| 단계 | 내용 | 핵심 산출물 |
|---|---|---|
| M0. 스캐폴드 | 모노레포(mise + pnpm + turbo) · Tauri+Vite+React 최소 실행 · CM6 마크다운 하이라이팅 · 디자인 시스템 토큰(Panda, → [디자인 시스템](design-system.md)) · 품질 게이트(oxlint/oxfmt/Vitest/lefthook, → [코드 품질](code-quality.md)) | `mise run dev`로 창이 뜨고 편집·하이라이팅 동작 · `mise run check` 통과 |
