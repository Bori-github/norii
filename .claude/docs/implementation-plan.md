...
읽기 전용 파일 저장 원자적 쓰기(rename)는 파일 권한(chmod 400)을 우회해 저장이 성공한다(rename은 디렉터리 권한만 검사 — E2E 실측). VS Code는 거부+명시적 덮어쓰기 — 차단 여부를 M2에서 결정 (→ file-lifecycle.md)
자동 저장 maxWait   순수 디바운스라 연속 타이핑 중에는 자동 저장이 무기한 연기됨 — 상한(예: 30초) 도입 여부. 정책 변경이므로 문서 선행 (→ file-lifecycle.md#자동-저장)
비활성 탭 충돌 UI  충돌 배너는 활성 탭 전용 — 비활성 탭 충돌은 dirty ●만 남는다. 탭 배지·전환 유도 등은 M2 watch(외부 변경 흐름)와 함께 설계 (→ file-lifecycle.md#외부-변경-처리)
