...
| M7. 상태 유지 | ✅ 완료 | 세션 복원(탭 목록 · 활성 탭 · 루트 폴더 · 탭별 커서와 자리 → [문서 모델](document-model.md#세션-복원)) · 뷰 모드·사이드바 접힘 저장 · 자동 저장 토글(→ [파일 생명주기](file-lifecycle.md#자동-저장)) | 껐다 켜면 열려 있던 탭과 폴더가 읽던 자리로 돌아온다 · 자동 저장을 끌 수 있다 |
