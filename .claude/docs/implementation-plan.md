...
| M6. 실사용 채우기 | ✅ 완료 | **손으로 쓰는 데 필요한 것들.** **사이드바 파일 조작**(새 파일/폴더 · 이름 변경 · 휴지통 삭제 → [Rust 커맨드 계약](rust-commands.md)) · 사이드바 접근성(ARIA 트리 시맨틱과 키보드 탐색 — 반쪽 ARIA는 없느니만 못하므로 한 세트로) · **한글 IME QA**(→ [한글 IME](korean-ime.md)) · **설정 화면**(테마 선택 · 유리 불투명도 · 흐림 반경)과 설정 저장 · 탭 ⚠ 배지 색 | 트리에서 파일을 만들고 지우고, 고친 설정이 다음 실행에도 남는다 |
| M7. 상태 유지 | ✅ 완료 | 세션 복원(탭 목록 · 활성 탭 · 루트 폴더 · 탭별 커서와 자리 → [문서 모델](document-model.md#세션-복원)) · 뷰 모드·사이드바 접힘 저장 · 자동 저장 간격(→ [파일 생명주기](file-lifecycle.md#자동-저장)) | 껐다 켜면 열려 있던 탭과 폴더가 읽던 자리로 돌아온다 · 자동 저장 간격을 고르거나 끌 수 있다 |
| M8. 시각 완성도 | ✅ 완료 | **전체 UI 쇄신** — 창 모서리·배경·크롬(타이틀바·사이드바·탭바·상태바)을 한 벌로 다시 맞춘다 · 타이포·간격 스케일 확정 · 프리뷰 타이포/상호작용 · 스크롤바 전역 스타일 | 화면 전체가 한 벌로 읽히고, 크기와 간격이 한 스케일에서 나온다 |
| M9. 폴더 없는 상태 | ✅ 완료 | **폴더를 열지 않았을 때 새 파일을 만드는 길을 정한다.** 새 파일은 연 폴더 안에 만드는데(→ [문서 모델](document-model.md#파일-트리-사이드바)) 폴더가 없으면 트리도 그 버튼도 없다 — `Cmd+N`이 여는 경로 없는 문서와 이 길의 관계를 정하고, 폴더를 열지 않은 화면에서 새 파일로 가는 길을 만든다 · **최근 파일**(`WorkspaceState.recentFiles`는 [문서 모델](document-model.md#상태-구조)이 규정하나 구현이 없다) — 그 화면에서 다시 열 곳을 보인다 | 폴더를 열지 않고 앱을 켜도 새 문서를 만들어 저장까지 가고, 최근에 연 파일을 그 자리에서 다시 연다 |
| M10. 문서 속 이미지 | ✅ 완료 | **문서 옆에 둔 이미지와 원격 이미지를 프리뷰에 띄운다.** 경로 해석·sanitize 경계·실패 표시는 [프리뷰 전략 — 이미지](preview-strategy.md#이미지)가, 로컬 이미지가 지나는 통로의 경로 제한은 [Rust 커맨드 계약 — 권한](rust-commands.md#권한-capabilities)이 확정했다 | 폴더를 열지 않아도 문서 옆 이미지가 프리뷰에 뜨고, 없는 이미지는 그 자리에 알린다 |
| M11. 성능·규모 | 예정 | 대형 폴더 트리 가상화(현재 미도입 → [문서 모델](document-model.md#파일-트리-사이드바)) · 프리뷰 렌더 후속(→ [프리뷰 전략](preview-strategy.md#디바운스)) | 파일이 수천 개인 폴더와 긴 문서에서도 즉시 반응한다 |
| M12. 배포(mac) | 예정 | 코드 서명·공증 · 자동 업데이트 · 번들 크기 목표 확정 | 배포 가능한 서명된 mac 빌드 |
...
>
> 테마(라이트/다크 · CM6 테마 토큰 통합)와 **창 유리**(투명 창 + 창 뒤 흐림 → [창 표면 계약](design/window-chrome.md))는 원래 다듬기 항목이었으나 별도 트랙에서 구현했다. 그 조각들 — **설정 화면**(테마 선택 · 유리 값 조절)과 저장 — 은 M6이 맡는다.
...
- M6 → [디자인 시스템 — 테마](design/design-system.md#테마-라이트다크) · [창 표면 계약](design/window-chrome.md) · [파일 생명주기 — 설정 저장](file-lifecycle.md#설정-저장) · [Rust 커맨드 계약](rust-commands.md) · [에디터 전략 — 단축키 계약](editor-strategy.md#단축키-계약) · [한글 IME](korean-ime.md)
- M7 → [문서 모델 — 세션 복원](document-model.md#세션-복원) · [파일 생명주기 — 자동 저장](file-lifecycle.md#자동-저장)
- M8 → [타이포](design/decisions/typography.md) · [디자인 시스템](design/design-system.md) · [프리뷰 전략](preview-strategy.md)
- M9 → [문서 모델 — 빈 탭](document-model.md#빈-탭--탭바는-비지-않는다) · [문서 모델 — 최근 파일](document-model.md#최근-파일) · [Rust 커맨드 계약 — 세션·다이얼로그](rust-commands.md#세션)
- M10 → [프리뷰 전략 — 이미지](preview-strategy.md#이미지) · [보안](security.md)(CSP `img-src` · asset 프로토콜 스코프) · [Rust 커맨드 계약 — 권한](rust-commands.md#권한-capabilities)
- M11 → [플랫폼 전략](platform-strategy.md#배포-경로--app-store는-비목표)
...
- **사이드바 밖의 새 파일 버튼** — 탭바에 새 파일 버튼을 둘지, 두면 이름을 어디서 받을지 — 사이드바를 접으면 새 파일을 만드는 길이 화면에서 사라진다 (→ [문서 모델](document-model.md#파일-트리-사이드바))
- **접힘 상태 영속화** — 사이드카 도입 시점 (→ [에디터 전략](editor-strategy.md) · [비목표](../rules/non-goals.md))
- **창 크기·위치 복원** — 도입 여부와 방식 (→ [문서 모델](document-model.md#세션-복원))
- **프리뷰 스크롤** — 프리뷰 자리를 탭별로 따로 기억할지 — 현재는 프리뷰가 편집면의 자리를 따라간다(→ [프리뷰 전략](preview-strategy.md#스크롤-동기화))
...
- **원격 이미지 끄기** — 원격 이미지를 끄는 설정을 둘지 — CSP는 정적이라 껐다 켜려면 렌더 단계에서 따로 제거해야 한다 (→ [보안](security.md#1-tauri-csp-content-security-policy) · [프리뷰 전략](preview-strategy.md#원격-이미지))
- **허용된 폴더 밖의 이미지** — 연 폴더에도 문서 폴더에도 없는 이미지를 보이게 할지. 다이얼로그로 이미지 폴더를 추가 허용하는 길을 둘지가 결정 대상이다 (→ [프리뷰 전략 — 어느 폴더의 이미지를 읽는가](preview-strategy.md#어느-폴더의-이미지를-읽는가))
...
- **중복 정리** — 테스트의 IpcError 목 6곳을 공용 헬퍼로 (→ [프론트엔드 아키텍처](frontend-architecture.md))
- **들여쓰기 상수화** — 트리 들여쓰기 `12`/`8`px에 이름 부여 — M8 간격 토큰 확정과 함께
...
- **저장 다이얼로그 파일명 제안** — rfd 제약으로 뺀 우회를 업스트림이 고치면 제거한다 (→ [Rust 커맨드 계약](rust-commands.md#다이얼로그))
