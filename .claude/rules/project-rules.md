...
- **변경 크기 상한** — 한 검토 단위(보고→승인→커밋)가 **신규·수정 파일 8개 또는 ±300줄을 초과할 것으로 예상되면**, 작업을 시작하기 전에 멈추고 분할 계획(각 단위의 내용·순서)을 먼저 보고한다. 상한 이하의 작업은 한 번에 진행한다. 의도는 검토 가능한 크기 유지다 — 한 번에 파일 수십 개가 쌓이면 사용자가 변경을 실질적으로 검토할 수 없다.
